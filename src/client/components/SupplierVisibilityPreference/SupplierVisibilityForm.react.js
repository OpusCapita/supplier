...
const TYPES = ['public', 'private', 'businessPartners'];

class SupplierVisibilityForm extends Component {
  static propTypes = {
  ...
  selectOptions = (fieldOptions) => {
    return fieldOptions.map(option => {
      const label = this.context.i18n.getMessage(`Supplier.Visibility.Type.${option}`);
  ...
  renderField = (attrs) => {
    return (
      <AttributeValueEditorRow labelText={ this.context.i18n.getMessage(`Supplier.Visibility.Label.${attrs.fieldName}`) } >
        { attrs.component }
      ...
      <form className="form-horizontal">
        { this.renderField({
            ...
            component: (
              ...
              >
                {this.selectOptions(TYPES).map((item, index) => {
...
export default SupplierVisibilityForm;
